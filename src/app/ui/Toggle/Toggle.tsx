...
const Toggle: FC<Props> = ({
  ...
  return (
    <div
      ...
      title={`Toggle ${enabled ? 'Dark' : 'Light'} Mode`}
      onClick={() => {
        setEnabled(!enabled);
      }}
      className={`toggle-track ${trackClasses} ${duration} ${
        enabled ? bgDark : bgLight
      }`}
    >
      <div
        className={`toggle-thumb ${thumbClasses} ${
          enabled ? bgLight : `translate-x-full ${bgDark}`
        }`}
      >
        <svg
          className={`toggle-icon ${iconClasses} ${
            enabled ? fillDark : fillLight
          }`}
